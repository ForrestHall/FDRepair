...
<head>
    ...
    <style>
        .hero-gradient { background: linear-gradient(135deg, rgba(30, 64, 175, 0.9) 0%, rgba(13, 148, 136, 0.85) 100%); }
        ...
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center py-4">
                <a href="./" class="text-2xl font-bold text-slate-800 flex items-center gap-2">
                    <svg class="w-8 h-8 text-fdr-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="1" y="3" width="15" height="13" rx="2" ry="2" stroke-width="2"></rect><path d="M16 8h4l3 3v5h-7V8z" stroke-width="2"></path><circle cx="5.5" cy="18.5" r="2.5" stroke-width="2"></circle><circle cx="18.5" cy="18.5" r="2.5" stroke-width="2"></circle></svg>
                    <span><?php echo htmlspecialchars($siteName); ?></span>
                </a>
                <div class="hidden md:flex items-center space-x-6">
                    <a href="./" class="text-fdr-blue font-semibold">Home</a>
                </div>
                <button id="mobile-menu-button" class="md:hidden text-slate-600 hover:text-fdr-blue focus:outline-none" aria-label="Toggle menu" aria-expanded="false">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                </button>
            </div>
        </div>
        <div id="mobile-menu" class="hidden md:hidden border-t border-slate-100">
            <a href="./" class="block py-3 px-4 text-fdr-blue font-semibold bg-slate-50">Home</a>
        </div>
    </nav>
        ...
        <section class="relative text-white text-center bg-slate-700 min-h-[32rem] flex items-center" aria-labelledby="hero-heading">
            ...
            <div class="relative container mx-auto px-4 py-20 md:py-28 w-full">
                <h1 id="hero-heading" class="text-4xl md:text-5xl lg:text-6xl font-extrabold leading-tight max-w-4xl mx-auto">
                    Enter Your ZIP – Find the <span class="text-cyan-300">Closest Diesel Repair</span>
                </h1>
                    ...
                    <form class="zip-form space-y-4" id="searchForm" role="search" aria-label="Find closest diesel repair by ZIP code">
                        ...
                        <fieldset class="text-center text-white">
                            ...
                            <div class="flex flex-wrap justify-center gap-x-6 gap-y-2">
                                <label class="flex items-center space-x-2 cursor-pointer"><input type="radio" name="dist" value="25" class="h-4 w-4 text-fdr-blue"> <span>25 mi</span></label>
                                <label class="flex items-center space-x-2 cursor-pointer"><input type="radio" name="dist" value="50" checked class="h-4 w-4 text-fdr-blue"> <span>50 mi</span></label>
                                <label class="flex items-center space-x-2 cursor-pointer"><input type="radio" name="dist" value="100" class="h-4 w-4 text-fdr-blue"> <span>100 mi</span></label>
                            </div>
                        </fieldset>
                        <button type="submit" class="w-full bg-cyan-500 text-white font-bold py-4 px-6 rounded-lg shadow-lg hover:bg-cyan-600 transition">Find Closest Diesel Repair</button>
                    </form>
    ...
    $(function(){
        ...
        function renderCard(item){
            ...
            return '<article class="rescard bg-white rounded-xl shadow-sm p-5 flex flex-col justify-between card-hover">' +
                ...
                '<h3 class="text-lg font-bold text-slate-800">' + (item.SITE ? '<a class="hover:text-fdr-blue transition" href="' + item.SITE + '" target="_blank" rel="noopener">' : '') + (item.NAME || '') + (item.SITE ? '</a>' : '') + '</h3>' +
                ...
                (item.PHONE ? '<a class="bg-fdr-blue text-white px-4 py-2 rounded-lg font-semibold flex items-center gap-2 hover:bg-fdr-blue-dark transition" href="tel:' + item.PHONE.replace(/\D/g,'') + '">Call</a>' : '') +
                '</div></article>';
        }
        ...
        $('#searchForm').submit(function(e){
            ...
            $('.results-container').html('<div class="col-span-full text-center py-8"><div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-fdr-blue border-t-transparent"></div><p class="mt-2 text-slate-600">Finding closest diesel repair...</p></div>');
        ...
        $('.locate').click(function(){
            $('.results-container').html('<div class="col-span-full text-center py-8"><div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-fdr-blue border-t-transparent"></div><p class="mt-2 text-slate-600">Getting location...</p></div>');
